...
<style>
.cap{font-family:'Nanum Barun Gothic','나눔바른고딕',nbg,NanumGothic,'나눔고딕'}	
</style>
			...
			<div id="index">
				<section id="sect1" class="row w1140 mb30 mb15-xs pa0-xs">
					...
					<div class="col-sm-4 col-xs-12 pa30 text-center" style="<?=$this->session->userdata['email']? $display : '' ?>">
						...
						<a class="btn btn-lg btn-block bg_blue" href="/auth/regster">회원가입</a>
...
function onScroll(event){
    ...
    if(scrollPos >= sum){    	
    	$(".keyword_coll").addClass("m_keyword-fixed-top-fix");
    	$(".category_tab").addClass("m_cate-fixed-top-fix");    	
    }else{  	
    	$(".keyword_coll").removeClass("m_keyword-fixed-top-fix");
    	$(".category_tab").removeClass("m_cate-fixed-top-fix");
    }
}
</script>		
					<div class="col-xs-12">
				...
				<section id="sect2" class="row w1140 pb60">
					...
					<ul class="category_tab clearfix " style="z-index: 90">
						<li><a class="keyword_tg collapsed" href="#keyword" data-toggle="collapse" item="keyword" style="color: #fffff !important;" >키워드 <i class="glyphicon glyphicon-menu-up"></i></a></li>
									...
									<div class="cnt_wrp">
										...
										<div class="meta clearfix">
											...
											<div class="pull-right">
												...
												?>
												최종가 <strong class="color_red">￦<?=number_format($price)?><!-- &nbsp;&nbsp;/&nbsp;$<?=$val['usd']?> --></strong>
											</div>
										</div>
									</div>
								</div>
					...
					<!-- 아이템 목록 -->					
					<div id="" class="lst"><!-- lst_slideLst-->
						<ul class="row">
							<? $i=1; foreach($product_list2 as $key => $val): ?>
							<li class="col-sm-4 col-xs-12 itm">
								<div>
									<a href="#">
										<div id="img_<? echo $i++; ?>" class="m_vimeo_play" >
											<img class="img-responsive" src="<?=IMG_O_PATH.$val['imagePath']."/".$val['imageLFile']?>" alt="" item="<?=$val['movieVimeoId']?>">
											<div class="vimeo_play_movie"></div>
										</div>
									</a>
				...
				<section id="sect7" class="row pa60 pa20-xs text-center color_wh" style="<?=$this->session->userdata['email']?  $display : "display:block" ?>">
					<h1 class="h2"><strong>고품질 동영상의 중심지로 당신을 초대합니다.</strong></h1>
					<div class="pa40 pa20-xs">
