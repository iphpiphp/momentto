...
<div class="row side_page w1140">
	...
	<div id="ct" class="col-md-10">
		...
		This is Sample page! Test Acount: Open Graph Test User	171891906487909	open_zuyfciz_user@tfbnw.net [https://www.facebook.com/profile.php?id=100010013028023]
		<nav>
			<ul class="pagination">
				<?=$page_nation?>
			</ul>
					...
					<div style="min-height:603px">
						...
						<div class="cnt_wrp">
							...
							<div class="meta clearfix">
								<div class="pull-left pt5">Amt(count)</div>
								<div class="pull-right">
									<strong class="price color_red">￦<?=number_format($val['price'])?></strong> (1)
								</div>
							</div>
							<div class="summary">
								<? if($status_div == 'wait'){ ?>
...
When you click to start Movie Maker button below to start making the movie . Movies are available days <strong class="color_red"><?=$leftDate?></strong> days , starting with the cancellation / refund of the movie at the same time it is impossible.
								<? } ?>
									
								<? if($status_div == 'complete'){ ?>									
										<div class="row pt10">
										<div class="col-xs-12 pb4">
											<a class="btn btn-lg btn-block" href="<?=$val['filePath']?>/<?=$val['storeFile']?>">movie download</a>
										</div>
										<div class="col-xs-6 pr2">
											<a class="btn btn-lg btn_blk btn-block" href="javascript:link_make('<?=$val['filePath']?>/<?=$val['storeFile']?>', 'link');" >link</a>
										</div>
										<div class="col-xs-6 pl2">
											<a class="btn btn-lg btn_blk btn-block" href="javascript:link_make('<?=$val['filePath']?>/<?=$val['storeFile']?>', 'ife');">source</a>
										</div>
										</div>
										<div class="row pt10">
										<div class="col-xs-12 pr2 pb4">
											<a class="btn btn-lg btn-block bg_blue " href="/facebook/fileupload/<?=$val['id']?>">My Facebook upload</a>
										</div>
									</div>
								<? } ?>
							...
							<div class="btnarea">
								...
								<? if($status_div == 'ready'){ ?>
									<?if($this->agent->is_mobile()){ ?>
										...
										<p class="pt10 text-center"><a class="color_grey pull-left" href="/customer/emailaq_view/"><u>movie Q&amp;A </u></a></p>	
									<? }else{ ?>
									<div><a class="btn btn-lg btn-block move_start" href="#" data-itemid="<?=$val['item_id']?>" data-orderid="<?=$val['orderId']?>">Movie Maker Start!</a></div>
									<p class="pt10 text-center"><a class="color_grey pull-left" href="/customer/emailaq_view/"><u>movie Q&amp;A </u></a></p>
									<? } ?>
								...
								<p class="pt10 clearfix">
									<a class="color_grey pull-left" href="/customer/emailaq_view/"><u>movie Q&amp;A </u></a>
									<a class="color_blue pull-right" href="/product/detail/<?=$val['pid']?>"><u>review edit</u></a>
								</p>
								<? } ?>
