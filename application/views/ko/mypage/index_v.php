...
<div id="container" class="clearfix">
	...
	<?=aside_top_mypage()?>
		...
		<!-- list2 -->
		<div class="lst v2">
			<div><ul class="pagination"><?=$page_nation?></ul></div>
			<ul class="row">
				<? 
 ...
 *
 */
/**	리본 정리 ribbon_blue = 파랑 ribbon_red = 빨강 ribbon_gray = 회색 **/
					...
					foreach($lists as $key => $val):
						...
						$leftDate = intval((strtotime($save_date)-strtotime(date('Y-m-d'))) / 86400); // 나머지 날짜값이 나옵니다.
	 					...
	 					$eDate = date('Y-m-d',strtotime($val['startDatetime']."+3 day"));
						$editDate = intval((strtotime($eDate)-strtotime(date('Y-m-d'))) / 86400); // 나머지 날짜값이 나옵니다.
						if($editDate < 0) $editDate = -1;
						...
						if($val['status'] == '03') $status_div = "progress";
						if($val['status'] == '08') $status_div = "del";
						if($val['status'] == '10') $status_div = "progress_10";
						if($val['status'] == '10' && $val['storeFile'] && $val['isComplete']) $status_div = "complete";
						...
						if($leftDate < 0 && $status_div != "complete") {
							$status_div = "del";
							$leftDate = 0;
						}
					...
					<li class="col-sm-6 col-xs-12 itm v2">
						<div>
							...
							<div class="cnt_wrp">
								...
								<? if($status_div == 'complete'){ ?>
										<? if($editDate >= 0 && $val['reset_avail']>=1) { ?>
											<h5 class="text-center">수정잔여일수 : <strong class="color_red"><?=$editDate?>일</strong></h5>
										<? } ?>
									<div class="row">
										<div class="col-xs-12 pb4">
											<a class="btn bg_red btn-block" href="<?=$val['filePath']?>/<?=$val['storeFile']?>">무비 다운로드</a>
										</div>
										<div class="col-xs-6 pr2 pb2">
											<a class="btn btn-block" href="javascript:link_make('<?=$val['filePath']?>/<?=$val['storeFile']?>', 'link');">링크</a>
										</div>
										<div class="col-xs-6 pl2 pb2">
											<a class="btn btn-block" href="javascript:link_make('<?=$val['filePath']?>/<?=$val['storeFile']?>', 'ife');">소스</a>
										</div>
										<? if($editDate >= 0 && $val['reset_avail']>=1) { ?>
										<div class="col-xs-12 pb2">
											<a class="btn btn-lg btn-block move_edit" href="javascript:;" data-editDate=<?=$editDate?> data-avail="<?=$val['reset_avail']?>" data-itemid="<?=$val['item_id']?>" data-orderid="<?=$val['orderId']?>">수정하기</a>
										</div>
										<? } ?>
									</div>
								<? } ?>
								<? if($status_div == 'del'){ ?>
									<p class="fs14 color_blue2">보증기간이 만료되었습니다.</p>
								<? } ?>
								...
								<div class="btnarea">
									...
									<? if($status_div == 'ready'){ ?>
										<?if($this->agent->is_mobile()){ ?>
											<div>모바일 환경에서는 제작이 불가능 합니다. <br>PC로 접속 해 주십시오. </div>
											<p><a class="color_blue2" href="/customer/emailaq_view/"><u>무비 Q&amp;A 질문하기</u></a></p>
										<? }else{ ?>
									...
									<? if($status_div == 'complete'){ ?>
										<p class="pt0 clearfix">
											<a class="color_blue2 pull-left" href="/customer/emailaq_view/"><u>무비 Q&amp;A 질문하기</u></a>
											<a class="color_blue2 pull-right" href="/product/detail/<?=$val['pid']?>"><u>평점/리뷰 작성하기</u></a>
										</p>
									<? } ?>
